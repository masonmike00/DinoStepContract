...
contract DinoStepToken is Context, IBEP20, Ownable {
    ...
    uint256 public biggestBuyRewardPercentage = (PERCENT_DENOMENATOR * 100) / 100; // 100%
    ...
    event UpdateFees(uint256 bnbFee, uint256 prFee, uint256 buyFee);
    ...
    constructor () {
        ...
        prPoolAddress = payable(0x5841542F69B253e23Ab5Efb541fFe506cc7A7a8c);
    ...
    function setModAddress(address account) external onlyOwner {
    ...
    function setFees(uint256 bnbFee, uint256 prFee, uint256 buyFee) external onlyOwner() {
        require(bnbFee + prFee <= 8  &&  buyFee <= 4);
        _BNBFee = bnbFee;
        _PRFee = prFee;
        _buyFee = buyFee;
        emit UpdateFees(bnbFee, prFee, buyFee);
    }
    ...
    function _getSellFeeValues(uint256 tAmount) private view returns (uint256) {
        ...
        uint256 fee = tAmount.mul(_BNBFee + _PRFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(fee);
        return tTransferAmount;
    }
    ...
    function getBiggestBuyer(uint256 hour) external view returns(address) {
        return biggestBuyer[hour];
    }

    function getBiggestBuyerAmount(uint256 hour) external view returns(uint256) {
        return biggestBuyerAmount[hour];
    }
    ...
    function _transfer(
        ...
        bool _isBuy = (from == pancakeswapV2Pair) && (to != address(pancakeswapV2Router));
        uint256 _hourAfterLaunch = getHour();
        if (_isBuy && amount > biggestBuyerAmount[_hourAfterLaunch]) {
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 _currentFee = _BNBFee.add(_PRFee);
